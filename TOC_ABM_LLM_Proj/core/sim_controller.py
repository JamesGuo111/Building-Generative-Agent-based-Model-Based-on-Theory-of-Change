...
class LearningPolicy:
    """
    学习更新策略（默认严格复刻你原本的 _learn_update 逻辑）：
      - base_gain = gain_per_qual * qual，随后乘以 100（把“很小的增量”映射到 0..100 标度）
      - 失败概率 p_fail = (100 - qual)/100 * p_fail_at_qual0
      - 失败惩罚 fail_penalty（默认 1 分）
      - 缺勤衰减 absent_decay（按 0..1 比例系数，最终乘以 100）

    NEW（你这次要的改动）：
      - 上学（update_learning）除了更新 L，也会更新就业技能 e_skill（0..100）
      - 这是 TOC 里“schooling -> skill（intermediate outcome）”的一个可控机制
      - TVET 的技能提升建议用 schedules（因为你现在把 TVET 当终点态，不走 update_learning）
    """
